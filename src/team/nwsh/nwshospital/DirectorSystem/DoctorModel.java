...
public class DoctorModel extends AbstractTableModel {
    ...
    public DoctorModel()
    {
		...
		RowData=new Vector(); 
		sql = "SELECT ACC_ID,ACC_NAME,(SELECT SEC_NAME FROM SECTIONS WHERE SECTIONS.SEC_ID=ACCOUNTS.SEC_ID),(SELECT IFNULL(SUM(IFNULL(RES_SUM,0)),0) FROM RESULTS WHERE ACCOUNTS.SEC_ID=(SELECT SEC_NAME FROM SECTIONS WHERE SECTIONS.SEC_ID=ACCOUNTS.SEC_ID)),(SELECT COUNT(*) FROM RESULTS WHERE ACCOUNTS.SEC_ID=(SELECT SEC_NAME FROM SECTIONS WHERE SECTIONS.SEC_ID=ACCOUNTS.SEC_ID)) FROM ACCOUNTS WHERE CAST(ACCOUNTS.ACC_ID AS UNSIGNED INT)>=500000";						// 此处填写要执行的语句
	    db = new MySQLConnect(sql);							// 新建一个数据库连接
	    try {
			ret = db.pst.executeQuery();					// 执行sql语句，得到结果集
			while (ret.next()) {
