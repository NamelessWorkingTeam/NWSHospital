...
public class SectionModel extends AbstractTableModel {
    ...
    public SectionModel()
    {
		...
		RowData=new Vector(); 
		sql = "SELECT SECTIONS.SEC_ID, SECTIONS.SEC_NAME, (SELECT IFNULL(SUM(IFNULL(RESULTS.RES_SUM,0)),0) FROM RESULTS WHERE RESULTS.ACC_ID = (SELECT ACC_ID FROM ACCOUNTS WHERE ACCOUNTS.SEC_ID = (SELECT SEC_ID FROM SECTIONS WHERE SECTIONS.SEC_NAME='普通外科'))),(SELECT COUNT(*) AS SEC_NUMBER FROM RESULTS WHERE RESULTS.ACC_ID = (SELECT ACC_ID FROM ACCOUNTS WHERE ACCOUNTS.SEC_ID = (SELECT SEC_ID FROM SECTIONS WHERE SECTIONS.SEC_NAME='普通外科'))) FROM SECTIONS";						// 此处填写要执行的语句
	    db = new MySQLConnect(sql);							// 新建一个数据库连接
	    try {
			ret = db.pst.executeQuery();					// 执行sql语句，得到结果集
			while (ret.next()) {
